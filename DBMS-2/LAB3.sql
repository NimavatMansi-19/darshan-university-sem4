...
--2. Create a Procedure that will accept Department Name and based on that gives 
--employees list who belongs to that department.
...
BEGIN
	...
	WHERE DepartmentID=
	(SELECT DepartmentID FROM Departments WHERE DepartmentName=@DEPTNAME)
END
PR_DETAILS 'HR'
...
CREATE OR ALTER PROCEDURE PR_PROJDETAILS
@PRJNAME VARCHAR(50),
@DEPTNAME VARCHAR(50)
AS
BEGIN
	SELECT * FROM Projects WHERE ProjectName=@PRJNAME AND DEPARTMENTID=
	(SELECT DEPARTMENTID FROM Departments WHERE DepartmentName=@DEPTNAME)
END
PR_PROJDETAILS 'PROJECT ALPHA' ,'IT'
...
CREATE OR ALTER PROCEDURE PR_SAL_BET_INT
@MIN_SALARY INT, 
@MAX_SALARY INT  
AS
BEGIN
...
CREATE OR ALTER PROCEDURE PR_INFO_BY_GENDER
@GENDER VARCHAR(50)
AS
BEGIN
	SELECT * FROM Employee 
...
BEGIN
	...
	WHERE D.Location LIKE'%'+@LOC+'%'
END
PR_LOC 'SAN'

--Part – C 
--9. Create a procedure that will accepts From Date & To Date and based on that 
--he/she will retrieve Project related data.  
...
BEGIN
	SELECT * 
    FROM Projects 
    WHERE DepartmentID IN (SELECT DepartmentID FROM Employee WHERE HIREDATE BETWEEN @FROM_DATE AND @TO_DATE)
END
PR_DATE '2010-06-15 ','2015-03-10 '
--10. Create a procedure in which user will enter project name & location and 
--based on that you must provide all data with Department Name, Manager Name with 
--Project Name & Starting Ending Dates.
CREATE OR ALTER PROCEDURE PR_DET
@PROJNAME VARCHAR(50),
@LOCATION VARCHAR(50)
AS
BEGIN
	...
	JOIN Projects P
	ON D.DepartmentID=P.DepartmentID
	WHERE P.ProjectName=@PROJNAME AND D.Location=@LOCATION

END
PR_DET 'Project Beta','San Francisco '